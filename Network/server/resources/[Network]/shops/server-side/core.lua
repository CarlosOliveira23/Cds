...
local shops = {
	...
	["Police"] = {
		["mode"] = "Buy",
		["type"] = "Police",
		["perm"] = "Police" and "Core",
		["List"] = {
			["gsrkit"]  = 35,
			["gdtkit"] = 35,
			["handcuff"] = 450,
			["vest"] = 525,
			["WEAPON_POLICE_AMMO"] = 4,
...
function Kaduzera.requestPerm(Type)
    ...
    if Passport then
        ...
        if shops[Type]["type"] == "Police" or shops[Type]["type"] == "MechanicBuy" then
			if not vRP.HasService(Passport, shops[Type]["perm"]) then
				TriggerClientEvent("Notify", source, "police", "Você precisa estar em serviço para abrir o Shop da Policia", 5000)
				return false
			end
		end		
        ...
        if shops[Type]["perm"] ~= nil then
            if not vRP.HasService(Passport, shops[Type]["perm"]) then
                TriggerClientEvent("Notify", source, "vermelho", "Você não tem permissão para abrir este Shops", 3000)
                return false
            end
        end
        ...
        TriggerClientEvent("Notify", source, "vermelho", "Você precisa de um passaporte para abrir o Shops", 3000)
        return false
    end
end
-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    if Passport then
        ...
        for k, v in pairs(shops[name]["List"]) do
            inventoryShop[#inventoryShop + 1] = {
                ...
                index = itemIndex(k),
                peso = itemWeight(k),
                economy = parseFormat(itemEconomy(k)),
                max = itemMaxAmount(k),
        ...
        for k, v in pairs(inventory) do
            ...
            if (splitName[1] == "WEAPON_COMBATPISTOL" or
    splitName[1] == "WEAPON_PISTOL" or
    splitName[1] == "WEAPON_HEAVYPISTOL" or
    splitName[1] == "WEAPON_COLTXM177" or
    splitName[1] == "WEAPON_CARBINERIFLE" or
    splitName[1] == "WEAPON_CARBINERIFLE_MK2" or
    splitName[1] == "WEAPON_ADVANCEDRIFLE" or
    splitName[1] == "WEAPON_PUMPSHOTGUN" or
    splitName[1] == "WEAPON_SMG" or
    splitName[1] == "WEAPON_SMG_MK2" or
    splitName[1] == "WEAPON_TACTICALRIFLE") and splitName[2] then
    local ownerPassport = vRP.GetOwner(k)

    if ownerPassport and vRP.Identity(ownerPassport) then
        if type(v["desc"]) == "string" then
            v["desc"] = v["desc"].."<br><description>Nome de registro: <green>"..vRP.FullName(ownerPassport).."</green>.</description>"
        else
            v["desc"] = "<br><description>Nome de registro: <green>"..vRP.FullName(ownerPassport).."</green>.</description>"
        end
    else
        -- Se o ownerPassport não tiver uma identidade válida, defina a descrição como uma mensagem de erro.
        v["desc"] = "<br><description>Identidade não encontrada</description>"
    end
end
