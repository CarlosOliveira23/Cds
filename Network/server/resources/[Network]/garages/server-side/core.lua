...
local Garages = {
	...
	["70"] = { name = "Core", payment = false, perm = "Core" },
	...
	["152"] = { name = "Mechanic", payment = true, perm = "Mechanic" }
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- SIGNALREMOVE
-----------------------------------------------------------------------------------------------------------------------------------------
...
function Kaduzera.Vehicles(Number)
	...
	if Passport and not exports["hud"]:Wanted(Passport) then
		-- Verificar permissão antes de permitir o acesso à garagem
		if Garages[Number]["perm"] then
    		local hasPermission = vRP.HasGroup(Passport, Garages[Number]["perm"])

    		if not hasPermission then
        	TriggerClientEvent("Notify", source, "vermelho", "Você não tem permissão para acessar esta garagem.", 5000)
			...
			for _,v in pairs(Consult) do
				if VehicleExist(v["vehicle"]) then
					if v["mode"] == "dismantle" and v["dismantle"] <= os.time() then
						...
						TriggerClientEvent("Notify",source,"azul","Estamos recuperando o veículo <b>"..VehicleName(v["vehicle"]).."</b>, espere um pouco para abrir a garagem.",10000)
						return false
					end

					if v["work"] == "false" then
						Vehicle[#Vehicle + 1] = { ["Model"] = v["vehicle"], ["name"] = VehicleName(v["vehicle"]), ["Tax"] = MinimalTimers(v["tax"] - os.time()), ["Rental"] = MinimalTimers(v["rental"] - os.time()), ["Mode"] = v["mode"] }
					end
				end
			end
		end
...
AddEventHandler("garages:Impound",function(vehName)
	...
	if Passport then
		...
		if vRP.Request(source,"A liberação do veículo tem o custo de <b>$"..parseFormat(VehiclePrice * 0.25).."</b> dólares, deseja prosseguir com a liberação do mesmo?") then
...
AddEventHandler("garages:Sell",function(Name)
	...
	if Passport then
		...
		if Consult[1] then
			...
			if vRP.Request(source,"Vender o veículo <b>"..VehicleName(Name).."</b> por <b>$"..parseFormat(Price).."</b>?") then
				local Consult = vRP.Query("vehicles/selectVehicles",{ Passport = Passport, vehicle = Name })
				if Consult[1] then
	...
	if Passport then
		...
		if myVehicle[1] then
			...
			if Keyboard then
				...
				if Identity then
					if vRP.Request(source,"Transferir o veículo <b>"..VehicleName(Name).."</b> para <b>"..Identity["name"].." "..Identity["name2"].."</b>?") then
		...
		if not vehicle[1] then
			if parseInt(Gemstone) > 0 then
				...
				if parseInt(VehiclePrice) > 0 then
					if vRP.Request(source,"Comprar <b>"..VehicleName(Name).."</b> por <b>$"..parseFormat(VehiclePrice).."</b> dólares?") then
		...
		if vehicle[1] then
			...
			if Spawn[Plate] then
				if not Signal[Plate] then
					...
					if os.time() >= parseInt(Searched[Passport]) then
						...
						if DoesEntityExist(Network) and not IsPedAPlayer(Network) and GetEntityType(Network) == 2 then
							...
							TriggerClientEvent("Notify",source,"verde","Rastreador do veículo foi ativado por <b>30</b> segundos, lembrando que se o mesmo estiver em movimento a localização pode ser imprecisa.",10000)
						else
							if Spawn[Plate] then
								Spawn[Plate] = nil
							end
						...
						TriggerClientEvent("Notify",source,"azul","Rastreador só pode ser ativado a cada <b>60</b> segundos.",5000)
					end
				else
				...
				elseif vehicle[1]["arrest"] >= os.time() then
					TriggerClientEvent("Notify",source,"amarelo","Veículo apreendido, dirija-se até o <b>Impound</b> e efetue o pagamento da liberação do mesmo.",10000)
				else
					if vehicle[1]["rental"] ~= 0 then
						if vehicle[1]["rental"] <= os.time() then
							if vRP.Request(source,"Atualizar o aluguel do veículo <b>"..VehicleName(Name).."</b> por <b>"..Gemstone.." gemas</b>?") then
								if vRP.PaymentGems(Passport,Gemstone) then
						...
						if Garages[Number]["payment"] then
							if vRP.UserPremium(Passport) then
								...
								elseif vehicle[1]["mode"] == "normal" then
									local VehiclePrice = VehiclePrice(Name)
									if vRP.Request(source,"Retirar o veículo por <b>$"..parseFormat(VehiclePrice * 0.05).."</b> dólares?") then
...
AddEventHandler("garages:LockVehicle",function(source,Network)
	...
	if parseInt(Doors) <= 1 then
		TriggerClientEvent("Notify",source,"verde","O veículo foi <b>trancado</b>.",5000)
		TriggerClientEvent("sounds:source",source,"locked",0.7)
		SetVehicleDoorsLocked(Network,2)
	else
		TriggerClientEvent("Notify",source,"amarelo","O veículo foi <b>destrancado</b>.",5000)
		...
		if string.sub(Plate,1,4) == "DISM" then
			...
			if source then
				if Player(source)["state"]["Dismantle"] then
					...
					if Vehicle[1] then
						TriggerClientEvent("Notify",source,"amarelo","Este veículo não foi adicionado em sua garagem pois você já possui um <b>"..VehicleName(Player(source)["state"]["DismantleModel"]).."</b>.",10000)
					else
						TriggerClientEvent("Notify",source,"azul","Você escondeu o veículo <b>"..VehicleName(Player(source)["state"]["DismantleModel"]).."</b> em sua garagem, o mesmo pode ser usado durante um período de <b>24 Horas</b>.",10000)
			...
			if #(Coords - exports["propertys"]:Coords(Name)) <= 25 then
				...
				if Application then
					if #(Coords - exports["propertys"]:Coords(Name)) <= 25 then
						...
						TriggerClientEvent("Notify",source,"vermelho","A garagem precisa ser próximo da entrada.",5000)
					end
				end
			else
				TriggerClientEvent("Notify",source,"vermelho","A garagem precisa ser próximo da entrada.",5000)
