	...
	$(".populated").tooltip({
		create: function(event,ui){
			...
			if (name == "Passaporte" || name == "Distintivo"){
				...
				var Rolepass = $(this).attr("data-Rolepass");
				var Gemstone = $(this).attr("data-Gemstone");

				contents = `<item>${name} - ${Passport}</item>${description !== "false" ? "<br><description>"+description+"</description>":""}<br><legenda>Nome: <r>${Name}</r><br>Tipo Sangüineo: <r>${Blood}</r><br>Rolepass: <r>${Rolepass}</r><br>Diamantes: <r>${Gemstone}</r><br>Validade: <r>${Vality}</r><br>Premium: <r>${Premium}</r><br>Diamantes: <r>${Gemstone}</r></legenda>`;
			}
	...
	$.post("http://inventory/requestInventory",JSON.stringify({}),(data) => {
		...
		for (let x = 1; x <= data["invMaxpeso"]; x++){
			...
			if (data["inventario"][slot] !== undefined){
				...
				const item = `<div class="item populated" title="" data-max="${v["max"]}" data-economy="${v["economy"]}" data-description="${v["desc"]}" style="background-image: url('nui://vrp/config/inventory/${v["index"]}.png'); background-position: center; background-repeat: no-repeat;" data-amount="${v["amount"]}" data-peso="${v["peso"]}" data-item-key="${v["key"]}" data-name-key="${v["name"]}" data-slot="${slot}" data-Name="${v["Name"]}" data-Blood="${v["Blood"]}" data-Premium="${v["Premium"]}" data-Vality="${v["Vality"]}" data-Rolepass="${v["Rolepass"]}" data-Gemstone="${v["Gemstone"]}" data-Suitcase="${v["Suitcase"]}" data-Vehkey="${v["Vehkey"]}" data-Passport="${v["Passport"]}">
					<div class="top">
