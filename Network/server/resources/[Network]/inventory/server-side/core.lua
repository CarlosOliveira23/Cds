...
--vLOCKPICKING = Tunnel.getInterface("lockpicking")
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- KADUZERA VRIFY LOCAL FAVELAS TABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local FavelaCds = {
	{1506.14,-2473.23,72.62},
	{1273.81,-124.82,87.62},
	{1428.04,-768.9,73.25},
	{-3122.04,1790.39,22.02},
	{-3091.58,1423.89,33.38}
}
function Kaduzera.FavelaDistance(source)
	local Ped = GetPlayerPed(source)
	local Coords = GetEntityCoords(Ped)

	for k,v in pairs(FavelaCds) do
...
-- vRP.MEMORY
-----------------------------------------------------------------------------------------------------------------------------------------
function Kaduzera.Memory(source)
...
StealPeds = {
	...
	{ ["item"] = "WEAPON_PISTOL_AMMO", ["min"] = 45, ["max"] = 70 },
						...
						if Identity then
							...
							if Number == Passport and Split[1] == "identity" then
								if Identity["premium"] > os.time() then
									...
									v["Vality"] = MinimalTimers(Identity["premium"] - os.time())
									v["Premium"] = Groups[Number]
								end
...
function Kaduzera.Drops(Item, Slot, Amount, x, y, z)
    ...
    if Passport then
        if not Active[Passport] and not Player(source)["state"]["Handcuff"] and not exports["hud"]:Wanted(Passport) and not vRP.InsideVehicle(source) and GetPlayerRoutingBucket(source) < 900000 then
            ...
            -- Adicione uma verificação para itens danificados
        ...
        if not Active[Passport] and not Player(source)["state"]["Handcuff"] and not exports["hud"]:Wanted(Passport) and not vRP.InsideVehicle(source) and GetPlayerRoutingBucket(source) < 900000 then
            ...
            if vRP.Request(source,"Tem Certeza que Deseja Jogar "..itemName(Item).." no Lixo") then
                if vRP.TakeItem(Passport, Full, Amount, true, Slot) then
                    ...
                    TriggerEvent("Discord", "InventoryLixo", "**Passaporte:** " .. Passport .. "\n**Jogou Lixo:** " .. Amount .. "x " .. itemName(Item), 13541152)
                end
            else
                TriggerClientEvent("Notify", source, "vermelho", "Tempo acabou e seu item voltou para voce.", 5000)
            end
        end
    end
end
...
function Kaduzera.UseItem(Slot,Amount)
	...
	if Passport and not Active[Passport] then
		...
		if itemType(Full) == "Armamento" and parseInt(Slot) <= 5 and not Player(source)["state"]["Safezone"] and not Player(source)["state"]["TagCastigo"] then
			...
			if not verifyAnimals[Passport] and not Active[Passport] and Animals[Model][netObjects] < 5 then
				if (vRP.InventoryWeight(Passport) + itemWeight("meat")) <= vRP.GetWeight(Passport) then
					if vTASKBAR.taskOne(source) then
						...
						local BonusList = { "fabric","fabric","fabric" }
						local HuntList1star = { "meat","meat","meat" }
							...
							if os.time() >= parseInt(Active[Passport]) then
								...
								if Animals[Model] then
									...
									elseif parseInt(Animals[Model][netObjects]) >= 2 then
										...
										vRP.GenerateItem(Passport,"fabric",2,true)
									end
								end
							end
	...
	if Passport and not Active[Passport] then
		...
		repeat
			if os.time() >= parseInt(Active[Passport]) then
				...
				local AmountItens = math.random(150, 150)
				...
				if Class == 1 then
					...
					AmountItens = math.random(2000, 2500)
				elseif Class == 2 then
					VehRandom = math.random(4500)
					AmountItens = math.random(2000, 2500)
				elseif Class == 3 then
...
-- EXPERIENCE
-----------------------------------------------------------------------------------------------------------------------------------------
function Kaduzera.Experience(Category)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
...
function Kaduzera.RegistersTimers(Number)
    ...
    local Passport = vRP.Passport(source)
    if Passport then
        local Service, Total = vRP.NumPermission("Admin")
        if Total <= 0 then
        ...
        -- Verificar a dimensão do jogador (deve ser 0)
        if GetPlayerRoutingBucket(source) ~= 0 then
            TriggerClientEvent("Notify", source, "vermelho", "Você deve estar na dimensão PADRÃO para realizar esta ação.", 5000)
            return
        end
        ...
        if vRP.ConsultItem(Passport, "lockpick", 1) then
            if Registers[Number] then
                if os.time() < Registers[Number] then
                    ...
                    TriggerClientEvent("Notify", source, "amarelo", "Caixa vazia, aguarde <b>" .. CooldownCaixinha .. "</b> segundos para roubar novamente", 5000)
                    return false
                else
                    InitRegisters(Number, source)
                    local success = Kaduzera.Memory(source)
                    if success then
                        return true
                    else
                        TriggerClientEvent("Notify", source, "vermelho", "Você não conseguiu Invadir o Sistema e o Roubo foi Cancelado", 5000)
                        return false
                    end
                end
            else
...
function InitRegisters(Number, source)
    Registers[Number] = os.time() + 5 -- Tempo de cooldown de 900 segundos, ou seja, 15 minutos

    local Ped = GetPlayerPed(source)
    local Coords = GetEntityCoords(Ped)

    local Service = vRP.NumPermission("Police") and vRP.NumPermission("Praia-1")
    for Passports, Sources in pairs(Service) do
        async(function()
...
-- REGISTERSPAY
-----------------------------------------------------------------------------------------------------------------------------------------
local ValorRegister = math.random(1950,2200)
...
function Kaduzera.RegistersPay()
	...
	if Passport then
		...
		Player(source)["state"]["Buttons"] = true
		TriggerClientEvent("inventory:Close",source)
		TriggerClientEvent("Progress",source,"Roubando",5000)
		...
		repeat
			if os.time() >= parseInt(Active[Passport]) then
				...
				if math.random(100) >= 50 then
					vRP.GenerateItem(Passport,"dollarsroll",ValorRegister,true)
				else
					vRP.GenerateItem(Passport,"dollars",ValorRegister,true)
				end
