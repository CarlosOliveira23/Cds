...
Use = {
			...
			if CoordsAtm then
				...
				if vRP.TakeItem(Passport,Full,1,true,Slot) then
					...
					if math.random(100) >= 50 then
						Kaduzera.DropServer(CoordsAtm,"dollars",math.random(8000,12000))
					else
						Kaduzera.DropServer(CoordsAtm,"dollarsroll",math.random(8000,12000))
					end
					--vRP.DropServer(CoordsAtm,"dollarsroll",math.random(8000,12000))
	...
	["badge01"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		TriggerClientEvent("inventory:Close",source)
		vRPC.CreateObjects(source,"prop_xmas_tree_int",49,28422,0.065,0.029,-0.035,80.0,-1.90,75.0)
	end,

	["badge02"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		TriggerClientEvent("inventory:Close",source)
		vRPC.CreateObjects(source,"paper_1_rcm_alt1-8","player_one_dual-8","prop_xmas_tree_int",49,28422,0.065,0.029,-0.035,80.0,-1.90,75.0)
	end,
	...
	["nitro2"] = function(source,Passport,Amount,Slot,Full,Item,Split)
	...
	["vest"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		-- Verifique se o jogador tem a permissão "Police"
		if vRP.HasPermission(source, "Police") then
			Active[Passport] = os.time() + 10
		else
	...
	["tablemeth"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		Player(source)["state"]["Buttons"] = true
		TriggerClientEvent("inventory:Close", source)
		local favela = Kaduzera.FavelaDistance(source)
	...
	["creators"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		...
		if vRP.TakeItem(Passport,Full,1,true,Slot) then
			TriggerClientEvent("barbershop:Open2",source,true)
		end
	end,

	["creators2"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if (vRP.InventoryWeight(Passport) + itemWeight(Full)) <= vRP.GetWeight(Passport) then
			if vRP.TakeItem(Passport,Full,1,true,Slot) then
				TriggerClientEvent("inventory:Update",source,"Backpack")
	...
	["premiumprata"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		if vRP.TakeItem(Passport, Full, 1, true, Slot) then
			TriggerClientEvent("inventory:Update", source, "Backpack")
			TriggerEvent("Salary:Add", Passport, "PremiumPrata", 1)
	...
	["premiumouro"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		if vRP.TakeItem(Passport, Full, 1, true, Slot) then
			TriggerClientEvent("inventory:Update", source, "Backpack")
			TriggerEvent("Salary:Add", Passport, "PremiumOuro", 1)
	...
	["premiumplatina"] = function(source,Passport,Amount,Slot,Full,Item,Split)
			if vRP.TakeItem(Passport,Full,1,true,Slot) then
				TriggerClientEvent("inventory:Update",source,"Backpack")
				TriggerEvent("Salary:Add",Passport,"Premium",1)
				vRP.SetPermission(Passport,"Premium",1)
				vRP.SetWeight(Passport,80)
			...
			if vRP.HasPermission(Passport,"Premium",1) and vRP.TakeItem(Passport,Full,1,true,Slot) then
				...
				if vRP.GetWeight(Passport) >= 180 then
					TriggerClientEvent("Notify",source,"amarelo","Sua mochila de <b>80kg</b> não foi adicionada pois você atingiu o limite.",10000)
					return
				end
	...
	["energetic"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		...
		Citizen.SetTimeout(10000, function()
			...
			-- Enviar notificação de que o efeito do energético acabou
