...
local Preset = {
    ...
    ["mp_f_freemode_01"] = {
        ["hat"] = { item = 117, texture = 20 },
        ["pants"] = { item = 231, texture = 0 },
        ["vest"] = { item = 0, texture = 0 },
        ["bracelet"] = { item = -1, texture = 0 },
        ["backpack"] = { item = 0, texture = 0 },
        ["decals"] = { item = 0, texture = 0 },
    ...
    if Passport then
        if actived[Passport] == nil then
            ...
            if Identity then
                ...
                TriggerEvent("Discord", "Ban", "**Policial ID:** " .. parseFormat(Passport) .. "\n**Prendeu Bandido ID:** " .. parseFormat(nuser_id) .. "\n**Toral de Serviços:** " .. parseFormat(services) .. "\n**Total de Multa:** $" .. parseFormat(fines) .. "\n**Horário:** " .. os.date("%H:%M:%S") .. "\n**Motivo:** " .. text, 13541152)

                -- Adicione a linha abaixo para fechar o tablet quando alguém for preso
                TriggerEvent("police:closeSystem")
            end
