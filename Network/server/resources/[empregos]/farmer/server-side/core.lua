		...
		if GlobalState["Work"] >= Objects[Number]["Time"] then
			...
			if Passport and not Active[Passport] then
				...
				if GetSelectedPedWeapon(Ped) == GetHashKey("WEAPON_HATCHET") then
					...
					if vRP.HasPermission(Passport, "Premium") or vRP.HasPermission(Passport, "PremiumPrata") or vRP.HasPermission(Passport, "PremiumOuro") then
                        Amount = Amount * 2
                    end
    ...
    if Objects[Number] then
        if GlobalState["Work"] >= Objects[Number]["Time"] then
            ...
            if Passport and not Active[Passport] then
                ...
                if GetSelectedPedWeapon(Ped) == GetHashKey("WEAPON_HATCHET") then
                    local Amount = math.random(3, 5)
                    -- Adicionando o benefício para VIP (duplicar a quantidade de woodlog)
                    if vRP.HasPermission(Passport, "Premium") or vRP.HasPermission(Passport, "PremiumPrata") or vRP.HasPermission(Passport, "PremiumOuro") then
                        Amount = Amount * 2
                    end
